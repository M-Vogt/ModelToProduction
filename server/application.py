...
def generate_data(names, min_vals, max_vals):
    ...
    mu = (
        np.mean([min_val + max_val for min_val, max_val in zip(min_vals, max_vals)]) / 2
    )
    sigma = (
        np.std([min_val + max_val for min_val, max_val in zip(min_vals, max_vals)]) / 6
    )
...
while True:
    ...
    if label == predicted_label:
        ...
        print("WARNING: Label and Predicted Label are different!")
        print()
    ...
    # Pause for a certain time interval between data points
    time.sleep(1)
